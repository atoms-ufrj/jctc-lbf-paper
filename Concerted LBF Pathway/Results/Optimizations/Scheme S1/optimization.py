...
def Spp(x): #derivative of Sp(x)
    return 0.0 if x < 0 else (0.0 if x > 1 else 60*(2*x**2 - 3*x + 1)*x)
...
def reweighting(x):
    ...
    return mixture.reweighting(
        potential=f'beta*({U})',
        properties={'Up': '(hpC - hpD)*U_BG + hpD*U_LJ + hpE*U_SC'},
